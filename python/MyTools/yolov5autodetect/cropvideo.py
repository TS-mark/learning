...
def videoCrop2image(video_path,save_path,start_time,end_time,gap):
    ...
    if isinstance(end_time,str):
        ...
        if end_time == -1: # -1就直接读视频长度
            frame_count = int(videoCapture.get(cv2.CAP_PROP_FRAME_COUNT))
            frame_rate = frame_rate = int(videoCapture.get(cv2.CAP_PROP_FPS))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-end_time', type = str,default="-1")
    ...
    gap = args.gap
    for vdp in os.listdir(video_path):
        videoCrop2image(vdp, save_path, start_time, end_time, gap)